...
const int NUM_MAXIMO_FICHAS = 5; //*************TOCA CAMBIARLO PARA EL EXTREMO*************
...
int main(){
    srand (time(NULL)); //ESTO AQUI, NO EN LA FUNCION RANDOM
    ...
    Texture textura_barra_inferior, textura_informacion_normal;
    if(not textura_barra_inferior.loadFromFile("Barrainferior_3enraya.png")) cerr << "ERROR loading tictactoe bottom menu" << endl;
    if(not textura_informacion_normal.loadFromFile("Informacion_3enraya_normal.png")) cerr << "ERROR loading tictactoe information menu" << endl;


    //****SPRITE****
    Sprite information(textura_informacion_normal);
    information.setOrigin(textura_informacion_normal.getSize().x/2.0f, textura_informacion_normal.getSize().y/2.0f);
...
restart_game: //etiqueta para un goto del final
    ...
    bool jug_circulo = random_bool(); //false es jugador1(empieza jugando cruz) y true es jugador2
            ...
            switch(e.type){
                ...
                case Event::MouseButtonPressed:
                    if(not partida_finalizada and not mostrando_informacion){ //para que no sirvan los clicks si se han puesto las 9 fichas o ya tenemos ganador. Y si se está mostrando la informacion por pantalla (usuario pulsando 'I') pues tampoco queremos que haga caso de los clicks
                        pair<bool,bool> cojo_resultado = board.Place_X_or_O(Vector2f(e.mouseButton.x, e.mouseButton.y), cruces, circulos, posCruz, posCirculo, jug_circulo, linea);
                        partida_finalizada = cojo_resultado.first;
                        bool hay_ganador = cojo_resultado.second;
                        if(jug_circulo and hay_ganador){//Si jug_circulo es true significa que a la siguiente le toca jugar circulo por tanto ESTA la acaba de jugar cruz. Entonces si la partida no esta finalizada es que cruz no ha ganado en esta ultima jugada y si sí está finalizada y hay ganador es sí ha ganado cruz
                            GANADOR_CRUZ = true;
                            ++puntuacion.scorePlayer1; //suma el score de cruz
                        }
                        else if(not jug_circulo and hay_ganador){ //la primera condicion no hace falta pero es para que al leerlo se entienda mejor: si la partida esta finalizada y no ha entrado en el anterior if significa que por narices esta primera condicion se cumplira
